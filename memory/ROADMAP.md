...
| 4 — Imaging metadata + filters, Data-quality indicators | ✅ Done | Patient-chart-scoped `DataQualityPanel` (8 deterministic rules, severity ladder, direct resolution links, non-mutating) + `ImagingCard` (allow-listed modalities, missing-modality pill, transient filters). Independent flag `clinicalRedesignPhase3Slice4`. |
