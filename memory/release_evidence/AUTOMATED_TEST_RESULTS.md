...
| Backend perf governance CI check Pytest (`test_check_perf_governance.py`) | 18 | PASS | Read-only guarantee (1), block statuses (valid approved / valid pending / stale draft / expired approval / approaching expiry / not-applicable template) (6), structural failures (duplicate ids / malformed marker / invalid ordering / mixed units / missing approval fields) (5), downstream references + duplicated numbers (2), JSON output (1), CLI modes — default permissive / strict / allow-pending-drafts (3). |
| Backend shared governance lib Pytest (`test_perf_gov_lib.py`) | 22 | PASS | Constants pinning (4), marker parsing (3), staleness boundaries (4), numeric parsing (5), ordering validator (3), reviewer fields (2), context tuple (1), downstream reference validator (2), deterministic output (2), run-context validator (3). Pins the stable shared vocabulary the four scripts now depend on. |
| **Combined backend Clinical + tooling (all governance scripts)** | **281+ / 281+** | **PASS** | 15-file targeted pytest run; 136/136 pass on the tooling-only subset (harness + seeder + draft + promotion + governance-check + shared lib). |
